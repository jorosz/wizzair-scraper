...
class WizzAPI
  ...
  def initialize (endpoint = nil, debug = false)
    ...
    if ! endpoint.nil?
      ...
      response = do_http("https://wizzair.com/")
      raise 'error finding homepage and no API endpoint given' unless response.is_a? Net::HTTPSuccess
       /var apiUrl = \"(#{URI.regexp})\"/i =~ response.body
      if $1
        @API_ENDPOINT = $1
        puts "Found API endpoint %s" % $1
      else
        raise 'Error finding API endpoint'
      end
    end
  end
  ...
  def search(origin, destination, departure_date, return_date=nil)
    ...
    response=do_http(@API_ENDPOINT+"/search/search",:post,data,headers)
    raise 'error getting search results %s' % response.body unless response.is_a? Net::HTTPSuccess
